...
#define WIFI_SSID "AVANI"
#define WIFI_PASSWORD "avaninu123"

// Insert Firebase project API Key
#define API_KEY "AIzaSyA3GVbJRyaSRhMZB2pXUNzvxJiD6CYjySM"

// Insert RTDB URLefine the RTDB URL */
#define DATABASE_URL "https://esp32test-5ff86-default-rtdb.firebaseio.com/"
...
float temp, humidity, bodytemperature;
...
void loop() {
  ...
  if (millis() - tsLastReport1 > REPORTING_PERIOD_MS1) {
    ...
    if (help_counter >= 1) {
      emergency_notif = 1;
      //Firebase.RTDB.setInt(&fbdo8, "Emergency Call/State", emergency_notif);
      help_counter = 0;
    } else {
      emergency_notif = 0;
      //Firebase.RTDB.setInt(&fbdo8, "Emergency Call/State", emergency_notif);
    ...
    if (Firebase.ready() && signupOK) {
      Firebase.RTDB.setInt(&fbdo1, "Body Parameters/Body Temperature", temperature);
      Firebase.RTDB.setInt(&fbdo2, "Peripherals/Room Temperature", temp);
      Firebase.RTDB.setInt(&fbdo3, "Peripherals/Room humidity", humidity);
      Firebase.RTDB.setInt(&fbdo4, "Gyro/Sleep", sleep_timer_start);
      Firebase.RTDB.setInt(&fbdo5, "Gyro/Interrupt", interrupt);
      Firebase.RTDB.setInt(&fbdo6, "Gyro/Total Sleep", total_light_sleep / 60);
      Firebase.RTDB.setInt(&fbdo7, "sensor/HeartBeat", BPM);
      Firebase.RTDB.setInt(&fbdo8, "sensor/SpO2", SpO2);
    }
    ...
    if (Firebase.ready() && signupOK) {
      ...
      if (Firebase.RTDB.setInt(&fbdo2, "Peripherals/Room Temperature", temp)) {
        Serial.println("PASSED");
      ...
      if (Firebase.RTDB.setInt(&fbdo3, "Peripherals/Room humidity", humidity)) {
        Serial.println("PASSED");
