...
void loop() {
  ...
  Serial.println(Mymessage1);
  tft.setTextColor(RED);
  showmsgXY(360, 80, 3, &FreeSans12pt7b, Mymessage1);
  */
  if (millis() - timer1 > 10) {
    ...
    // Clear the screen
    tft.fillRect(0, 0, 350, 320, BLACK);

    // Draw the graph
    for (int i = 0; i < numReadings - 1; i++) {
      int x1 = map(i, 0, numReadings - 1, 0, 350);
      int x2 = map(i + 1, 0, numReadings - 1, 0, 350);
      int y1 = map(readings[i], 0, 1023, 320, 0);
      int y2 = map(readings[i + 1], 0, 1023, 320, 0);
